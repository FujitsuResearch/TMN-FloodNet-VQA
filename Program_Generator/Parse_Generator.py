...
def program_generator(question, output_type = 'list'):
     ...
     function_list = {'image_classify':'Classify_Overall_Flood_Condition',
                      'count':'Count',
                      'exists':'Exists',
                      'filter_buildings_unary':'Filter(Buildings)',
                      'filter_road_unary':'Filter(Road)',
                      'filter_buildings_flooded':'Filter(Buildings, Flooded)',
                      'filter_buildings_non-flooded':'Filter(Buildings, Non_Flooded)',
                      'filter_road_flooded':'Filter(Road, Flooded)',
                      'filter_road_non-flooded':'Filter(Road, Non_Flooded)'}
     ...
     if 'overall' in special_word:
          ...
          if attribute != None and object!= None:
               program_list.append(function_list['filter_' + object + '_' + attribute])
               program_string += function_list['filter_' + object + '_' + attribute]
          elif attribute == None and object!= None:
               program_list.append(function_list['filter_' + object + '_unary'])
               program_string += function_list['filter_' + object + '_unary']
          if start == 'How':
