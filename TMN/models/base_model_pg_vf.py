...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.use_location_embed:
            ...
            self.image_location_embeddings = nn.Linear(5, config.v_hidden_size) # ext
...
class BaseTransformerWithExtractor(BertPreTrainedModel):
    def __init__(self, config, extractor=None):
        ...
        self.bert = BaseTransformer(config)
    ...
    def forward(
        ...
        vil_prediction = self.bert(input_ids, input_imgs, image_loc, token_type_ids, attention_mask, image_mask, co_attention_mask)
