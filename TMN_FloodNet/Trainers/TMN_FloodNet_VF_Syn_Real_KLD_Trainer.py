...
def main():
    ...
    syn_dataset = Synthetic_Dataset(dataroot = path_cfgs.dataset_path,
                                      height = args.im_height,
                                      width = args.im_width)
    
    real_dataset = FloodNetVQA(dataroot = path_cfgs.dataset_path,
                               partition = 'Train',
                               height = args.im_height,
                               width = args.im_width)
    ...
    TRAIN_DATA_LEN = 3000
    VAL_DATA_LEN = 500
    TEST_DATA_LEN = len(real_dataset) - (TRAIN_DATA_LEN + VAL_DATA_LEN)

    real_train_dataset, validation_dataset, test_dataset = random_split(real_dataset, [TRAIN_DATA_LEN, VAL_DATA_LEN, TEST_DATA_LEN])
    
    print("Synthetic Training Data Examples:",len(syn_dataset))
    print("Real Training Data Examples     :",len(real_train_dataset))
    ...
    kl_loss_fn = nn.KLDivLoss(log_target = True, reduction='batchmean')
    ...
    for epochId in range(int(start_epoch), int(num_train_epochs)):
        ...
        total_loss_tmp = 0
        step_tmp = 0
        global_kl_loss_tmp = 0
        global_loss_tmp = 0
        global_total_loss_tmp = 0
        global_matches_tmp = 0

        for step,(syn_batch,real_batch) in enumerate(zip(syn_train_data_loader, real_train_data_loader)):
            iterId = startIterID + step + (epochId * num_steps)
            
            if syn_batch != None and real_batch != None:
                
                # Synthetic Forward Pass
                if device != torch.device("cpu"):
                    syn_batch = tuple(t.cuda(device=device, non_blocking=True) for t in syn_batch)
                img, qs, qs_type, pg_str, arguments, answer_id = (syn_batch)
                ...
                syn_answer_id = answer_id.type(torch.LongTensor).to(device)
                syn_outputs, syn_pred =  model(img, spatials, image_mask, arguments, region_props=regions, image_info=img_info)
            ...
            loss = ce_loss_fn(syn_pred, syn_answer_id)
            # Total Loss
            ...
            # Calculation for Synthetic Dataset Classification
            logits = torch.max(syn_pred, 1)[1].data  # argmax
            count_matches = ((logits - answer_id) == 0).sum().float()

            matches_tmp += count_matches.item()
            kl_loss_tmp += kl_loss.item()
            loss_tmp += loss.item()
            total_loss_tmp += total_loss.item()
            step_tmp += 1

            total_loss.backward()

            if (step + 1) % gradient_accumulation_steps == 0 or (step + 1) == int(syn_dataset.num_dataset / train_batch_size):
                ...
                model.zero_grad()
                global_step += 1
                global_kl_loss_tmp += kl_loss_tmp
                global_loss_tmp += loss_tmp
                global_total_loss_tmp += total_loss_tmp
                ...
                if global_step % 20 == 0 and global_step != 0:
                    global_kl_loss_tmp = global_kl_loss_tmp / 20.0
                    global_loss_tmp = global_loss_tmp / 20.0
                    global_total_loss_tmp = global_total_loss_tmp / 20.0
                    global_matches_tmp = global_matches_tmp / (gradient_accumulation_steps * train_batch_size * 20.0)

                    print(f'Training Epoch:{epochId}, Step:{step}, Global Step:{global_step}, Accuracy:{"{:.4f}".format(global_matches_tmp)}, KLD Loss:{"{:.4f}".format(global_kl_loss_tmp)}, Total Loss:{"{:.4f}".format(global_total_loss_tmp)}| lr:{scheduler.get_last_lr()[0]}', flush=True)
        ...
        for step, batch in enumerate(validation_data_loader):
            ...
            loss = ce_loss_fn(pred, answer_id)
        ...
        if eval_score >= best_eval_score:
            ...
            output_model_file = os.path.join(savePath, "TMN_FloodNet_Syn_Real_KL_L" + str(args.num_module_layers) + '_Ep' + str(int(args.num_epochs)) + ".bin")
    ...
    for step, batch in enumerate(test_data_loader):
        ...
        arguments = arguments.type(torch.LongTensor).to(device)
        answer_id = answer_id.type(torch.LongTensor).to(device)
        _, pred =  Inference_Model(img, spatials, image_mask, arguments, region_props=regions, image_info=img_info)
